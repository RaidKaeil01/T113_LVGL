...
{
    /* ========== 0. 设置屏幕背景颜色 ========== */
    lv_obj_set_style_bg_color(lv_scr_act(), lv_color_hex(0xADD8E6), LV_PART_MAIN);  // 浅蓝色背景
    
    /* ========== 1. 头像区域（左上角）========== */
    lv_obj_t * avatar_container = lv_obj_create(lv_scr_act());
    lv_obj_set_size(avatar_container, 130, 130);  // 头像容器大小
    lv_obj_align(avatar_container, LV_ALIGN_TOP_LEFT, 50, 60);  // 左上角，偏移20px
    ...
    // TODO: 用户添加头像图片路径，例如：
    lv_img_set_src(img_avatar, "A:res/image/start/favicon.png");
...
lv_obj_set_size(info_container, 400, 150);           // 调整整体宽度
...
lv_obj_set_size(time_container, 200, 150);
lv_obj_align(time_container, LV_ALIGN_LEFT_MID, 0, 0);  // 左对齐,无偏移
...
lv_obj_align(label_time, LV_ALIGN_TOP_RIGHT, -5, 5);
...
lv_obj_align(label_date, LV_ALIGN_TOP_RIGHT, -5, 30);
...
lv_obj_align(label_week, LV_ALIGN_TOP_RIGHT,-5, 50);

/* ---------- 2.2 右侧：天气信息区 ---------- */
lv_obj_t * weather_container = lv_obj_create(info_container);
lv_obj_set_size(weather_container, 200, 150);
lv_obj_align(weather_container,LV_ALIGN_RIGHT_MID, 0, 0); // 右对齐,无偏移
...
// 天气图标占位（圆形）
lv_obj_t * img_weather = lv_obj_create(weather_container);
lv_obj_set_size(img_weather, 50, 50);
lv_obj_align(img_weather, LV_ALIGN_TOP_LEFT, 5,5);
lv_obj_set_style_bg_color(img_weather, lv_color_hex(0xF39C12), LV_PART_MAIN);
lv_obj_set_style_radius(img_weather, 30, LV_PART_MAIN);
lv_obj_set_style_border_width(img_weather, 0, LV_PART_MAIN);

// 图标内符号（临时）
lv_obj_t * icon_label = lv_label_create(img_weather);
lv_label_set_text(icon_label, LV_SYMBOL_CALL);
lv_obj_set_style_text_font(icon_label, &lv_font_montserrat_28, LV_PART_MAIN);
lv_obj_align(icon_label,LV_ALIGN_RIGHT_MID, 0, 0); 
...
lv_obj_align(label_weather, LV_ALIGN_TOP_LEFT,5,60);
    ...
    /* ========== 5. 创建定时器更新时间 ========== */
    lv_timer_t * timer = lv_timer_create(time_update_timer_cb, 1000, NULL);  // 每秒更新
    // 定时器将无限循环执行
    
    
    /* ========== 6. 添加点击事件跳转到其他页面（可选）========== */
    // 用户可以添加全屏触摸事件，点击后跳转到主页面
    // 示例：
